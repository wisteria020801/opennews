...
CHAT_ID = os.environ.get("CHAT_ID_F", "-1003590230315") # Use the correct group ID (same as others)
...
async def run_oracle():
    ...
    任务：
    1. 【去重与打分】：去除重复信息。为每条关键事件打分（1-10分，8分以上为特级）。
    2. 【链上异动】：如果出现 Whale Alert (大额转账)，请分析是否为砸盘/拉盘信号。
    3. 【宏观日历】：如果出现 Economic Calendar (CPI/Rate)，请提示交易时间点。
    4. 【跨平台情绪共振】：寻找微博/TikTok/X上的共同热点。
    5. 预测一个短期市场影响。
    
    请用中文回答，保持简练。格式：
    
    🔥 **[流媒体共振]**: [话题] (Weibo+TikTok+X)
    🐳 **[链上/宏观]**: [大额转账/数据发布] -> [多/空信号]
    🔴 **[关键事件]**: [事件] (震级: 9/10)
    📉/📈 **[市场影响]**: [预测]
    📝 **[昨日复盘]**: 昨日预警[事件] -> 实际[结果] (准确率: High)
    """
