...
async def run_oracle():
    ...
    任务：
    1. 【去重与打分】：去除重复信息（如Reuters和AP报道同一事件，只取一条）。为每条关键事件打分（1-10分，8分以上为特级）。
    2. 【跨平台情绪共振】：寻找微博(Weibo)、TikTok、X(Twitter)和小红书(Xiaohongshu)上同时出现的热点。
    3. 识别一个最关键的事件。
    4. 预测一个短期市场影响（如：黄金上涨，加密货币下跌）。
    5. 【回测复盘】：简单提及昨天预测的准确性（模拟）。
